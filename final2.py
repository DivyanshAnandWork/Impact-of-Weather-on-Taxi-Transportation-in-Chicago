...
print("\n Transformation: Joining the dataframes on the basis of date")
...
def mapper(record):
	...
	'bad_condition': int(record['fog']) | int(record['snow_ice_pellets']) | int(record['hail']) | int(record['thunder']) | int(record['tornado_funnel_cloud']),
        'total_trip_minutes': record['total_trip_minutes']
    }
...
def reducer(key, values):
    total_taxi_duration = 0
    total_records = 0

    for value in values:
        total_taxi_duration += value['taxi_demand']['total_trip_minutes']
        total_records += 1

    average_taxi_duration = total_taxi_duration / total_records if total_records > 0 else 0

    yield key, {'average_taxi_duration': average_taxi_duration}
...
transformed_output = [
    {
        'timestamp': key[0],
        'bad_condition': key[1],
        'average_taxi_duration': values['average_taxi_duration']
    }
