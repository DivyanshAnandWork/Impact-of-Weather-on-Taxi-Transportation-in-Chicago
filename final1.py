...
def mapper(record):
	...
	'bad_condition': int(record['fog']) | int(record['snow_ice_pellets']) | int(record['hail']) | int(record['thunder']) | int(record['tornado_funnel_cloud']),
        ...
        'total_tolls': record['total_tolls'],
        'total_extras': record['total_extras'],
        'trip_total': record['trip_total'],
        'total_trp_miles': record['total_trp_miles'],
...
def reducer(key, values):
    ...
    for value in values:
        total_taxi_demand += value['taxi_demand']['trip_total']
        total_records += 1
...
transformed_output = [
    {
        'timestamp': key[0],
        'bad_condition': key[1],
        'average_taxi_demand': values['average_taxi_demand']
    }
