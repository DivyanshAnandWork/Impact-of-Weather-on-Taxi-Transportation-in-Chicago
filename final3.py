...
################# KPI 1################
print("Map Reduce function started...")
# Mapper Function
def mapper(record):
    date = record['date']
    ...
    emit_key = (str(date), taxi_fare['bad_condition'])
    emit_value = {'weather_conditions': weather_conditions, 'taxi_fare': taxi_fare}
...
def reducer(key, values):
    ...
    for value in values:
        total_taxi_fare += value['taxi_fare']['total_fare']
        total_records += 1

    average_taxi_fare = total_taxi_fare / total_records if total_records > 0 else 0

    yield key, {'average_taxi_fare': average_taxi_fare}
...
transformed_output = [
    {
        'timestamp': key[0],
        'bad_condition': key[1],
        'average_taxi_fare': values['average_taxi_fare']
    }
